...
function create_docker {
  virt-install --name $NODE1_NAME --network=type=direct,source=enp8s0,source_mode=bridge,mac=$NODE1_MAC $COMMON_VIRT_OPTS --boot=network,hd
  virt-install --name $NODE2_NAME --network=type=direct,source=enp8s0,source_mode=bridge,mac=$NODE2_MAC $COMMON_VIRT_OPTS --boot=network,hd
  virt-install --name $NODE3_NAME --network=type=direct,source=enp8s0,source_mode=bridge,mac=$NODE3_MAC $COMMON_VIRT_OPTS --boot=network,hd
  virt-install --name $NODE4_NAME --network=type=direct,source=enp8s0,source_mode=bridge,mac=$NODE4_MAC $LONGHORN_VIRT_OPTS --boot=network,hd
}
