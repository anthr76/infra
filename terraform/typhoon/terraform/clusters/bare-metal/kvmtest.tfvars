cluster_name = "k8skvm"
matchbox_http_endpoint = "http://nwk1-app-1.rabbito.tech:8080"
os_channel = "flatcar-stable"
os_version = "2605.7.0"
controllers = [
    {name = "node1", mac = "52:54:00:00:00:a1", domain = "k8s-node-1.k8skvm.rabbito.tech"},
]
workers = [
  { name = "node2", mac = "52:54:00:00:00:a2", domain = "k8s-node-2.k8skvm.rabbito.tech"},
  { name = "node3", mac = "52:54:00:00:00:a3", domain = "k8s-node-3.k8skvm.rabbito.tech"},
  { name = "node4", mac = "52:54:00:00:00:a4", domain = "k8s-node-4.k8skvm.rabbito.tech"}
]
k8s_domain_name = "k8skvm.rabbito.tech"
ssh_authorized_key = "ssh-rsa  hello@anthonyrabbito.com"
networking = "calico"
install_disk = "/dev/vda"
kernel_args = [
  "usb-storage.quirks=152d:0578:u"
]
cached_install = true
enable_reporting = true